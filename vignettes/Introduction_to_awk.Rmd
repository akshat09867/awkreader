...
The awkreader package introduces three new methods:
...
In order to demonstrate the capabilities of the awkreader package, we have posted the following files to **(fill in link)**:
...
```{r }
data.path <- system.file("Data", package = "awkreader")

titanic.data <- list.files(path = data.path, full.names = TRUE)
pattern.fread(the.files = titanic.data, the.patterns = c("Female", "Child", "1st"), tf = c(T, T, F), connectors = c("or", "and"))
```
